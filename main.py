...
DOWNLOADS = str(Path.home() / "Downloads" / "reducedRenamed")
...
def compress_and_rename(input_path, output_folder):
    ...
    try:
        ...
        return new_filename, (old_size, new_size)
...
if uploaded_files and st.button("Process Uploads"):
    ...
    for file in uploaded_files:
        ...
        new_name, sizes = compress_and_rename(temp_path, output_folder)
        if new_name and sizes:
            results.append((file.name, new_name, sizes))
    ...
    if results:
        st.success(f"✅ Processed {len(results)} images. Saved in {output_folder}")
        for orig, new, (old_kb, new_kb) in results:
            st.write(f"{orig} → {new} | {old_kb}KB → {new_kb}KB")
        st.info(f"Processed images are in your Downloads folder: {output_folder}")
    else:
